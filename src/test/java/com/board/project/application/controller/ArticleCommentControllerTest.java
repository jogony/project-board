...
class ArticleCommentControllerTest {
    ...
    @MockBean
    private CreateArticleCommentUsacase createArticleCommentUsacase;
    ...
    @WithUserDetails(value = "kswTest", setupBefore = TestExecutionEvent.TEST_EXECUTION, userDetailsServiceBeanName = "userDetailsService")
    @DisplayName("[view][Post] 댓글 등록 - 정상 호출")
    @Test
    void givenArticleCommentInfo_whenRequesting_thenSavesNewArticleComment() throws Exception {
        // Given
        Long articleId = 1L;
        ArticleCommentRequest request = ArticleCommentRequest.of(articleId, "test comments");
        willDoNothing().given(createArticleCommentUsacase).saveArticleComment(any(ArticleCommentDto.class));
        // When & Then
        mockMvc.perform(
                post("/comments/new")
        ...
        then(createArticleCommentUsacase).should().saveArticleComment(any(ArticleCommentDto.class));
    }
